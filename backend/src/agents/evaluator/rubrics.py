...
class Criterion:
    ...
    name: str
    description: str
    max_score: float = 100.0  # Максимальный балл за критерий (0-100)
...
class Rubric:
    ...
    def validate(self) -> bool:
        ...
        # Упрощенная валидация: проверяем, что все критерии имеют max_score <= 100
        return all(criterion.max_score <= 100.0 for criterion in self.criteria)
...
class RubricSystem:
    ...
    def _initialize_rubrics(self) -> None:
        ...
        # Сумма max_score всех критериев = 100
    ...
    def get_rubric(self, assignment_type: AssignmentType) -> Rubric | None:
        """Получить рубрику для типа задания."""
        return self._rubrics.get(assignment_type)

    def calculate_score(self, assignment_type: AssignmentType, criterion_scores: dict[str, float]) -> dict[str, Any]:
        """
        Вычислить итоговый балл на основе рубрики.

        Упрощенная система оценивания: сумма max_score всех критериев = 100,
        итоговый балл = сумма баллов по всем критериям (без нормализации).

        Args:
            assignment_type: Тип задания
            criterion_scores: Словарь с баллами по каждому критерию {criterion_name: score}
                            Каждый балл должен быть в диапазоне 0 до max_score критерия

        Returns:
            Словарь с результатами оценки:
            - total_score: итоговый балл (0-100) - сумма баллов по всем критериям
        ...
        for criterion in rubric.criteria:
            ...
            # Ограничиваем балл диапазоном 0 до max_score критерия
            score = max(0.0, min(criterion.max_score, score))
            total_score += score
        ...
        # Итоговый балл = сумма баллов по всем критериям (без нормализации)
        # Ограничиваем итоговый балл диапазоном 0-100 для безопасности
...
""",
        ...
        return templates.get(assignment_type, 'Шаблон обратной связи для данного типа задания не настроен.')
