...
### Prefer SQLAlchemy Core over ORM

Use ORM **only** for schema definitions with typed models.
Write **all queries** in SQLAlchemy Core: explicit SQL, no lazy loading, no session traps, fully async-safe.
...
* Easy to extend with joins, CTEs, JSON aggregation
* Business logic decoupled from ORM state

#### Typed table definition (ORM allowed only here)
...
#### Real query (Core only)

```python
from sqlalchemy import select, func
from src.database import database, users, posts
...
async def get_user_with_stats(user_id: UUID):
    query = (
        ...
        .group_by(users.c.id, users.c.username, users.c.email)
    )

    return await database.fetch_one(query)
```

Short, explicit, zero magic.